  ...
  <div :class="['app-input-phone', { disabled }]" @click="focusInput">
    ...
    <div :class="['input-wrapper', { error }]">
      <div class="country-dd">
        ...
        <div v-if="displayOptions" v-click-outside="hideValues" :class="['options-list', { opened: displayOptions }]">
          ...
          <ul>
            ...
            <li
              v-else
              v-for="(countryCode, i) in dropdownOptions"
              :key="i"
              :class="['option', { 'selected-option': countryCode.code === selectedCountryCode.code }]"
              @click="selectCountry(countryCode.dial_code)"
            >
      ...
      <div class="prefix-input-wrapper">
        <div class="prefix">{{ selectedCountryCode.dial_code }}</div>
        <input
          type="number"
          ref="numberInput"
          class="phone-number"
...
export default {
  ...
  watch: {
    modelValue: {
      deep: true,
      handler() {
        if (!this.modelValue.prefix && !this.countryCodes.some((x) => x.dial_code == this.modelValue.prefix)) this.selectCountry("+47");
      },
    },
  },
  computed: {
    selectedCountryCode() {
      if (!this.modelValue?.prefix) return this.countryCodes.find((x) => x.dial_code === "+47");
      return this.countryCodes.find((x) => x.dial_code === this.modelValue?.prefix);
    },
    dropdownOptions() {
      if (this.countrySearch == "") return this.countryCodes;
      return this.countryCodes.filter(
        (c) =>
          c.code.includes(this.countrySearch.toLowerCase()) ||
          c.dial_code.includes(this.countrySearch.toLowerCase()) ||
          c.name?.toLowerCase().includes(this.countrySearch.toLowerCase())
      );
    },
  },
  mounted() {
    if (!this.modelValue?.prefix) {
      if (!this.modelValue.prefix && !this.countryCodes.some((x) => x.dial_code == this.modelValue.prefix)) this.selectCountry("+47");
    }
  },
  methods: {
    ...
    toggleOptions() {
      ...
      if (!this.displayOptions) this.$refs.numberInput.focus();
    },
    selectCountry(val) {
      this.displayOptions = false;
      this.$emit("update:modelValue", { ...this.modelValue, prefix: val });
    },
    focusInput() {
      this.$refs.numberInput.focus();
    },
    handleNumberChange() {
      ...
      this.$emit("update:modelValue", { prefix: this.selectedCountryCode.dial_code, number: val });
    },
  },
};
</script>
